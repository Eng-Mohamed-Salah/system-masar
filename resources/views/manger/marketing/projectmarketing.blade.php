...
@section('scripts')
    <script>
        function createCard(id, project_name, start_date, end_date, team, department, tmLeaderName, tmLeaderImage, notesmar,
                completion_percentage, file_marketing, linkTask,employee_link ,parentColumn) {
                    // const linkTaskAsString = linkTask.toString();
                    const linkTaskAsString = linkTask;
                ...
                card.innerHTML = `
                        ...
                        @if(auth()->user()->can('isAdmin') || auth()->user()->can('isManager'))
                            <div class="dropdown-content" onclick="stopPropagation(event)">
                                ...
                                <input class="form-control createLink" type="text" data-project-id="${id}" placeholder="Type Link ...">

                                <div class="deleteEdit">
                                    <form class="delete-button my-2" action="{{ url('/client/manager/marketing/Tasks/') }}/${id}" method="post">
                                        @csrf
                                        @method('DELETE')
                                    ...
                                    <a href="{{ url('/client/manager/marketing/editTasks/') }}/${id}" class="edit-button my-2">
                        ...
                        <label class="searchDate">Created : <span>${start_date}</span></label>
                        ...
                        <label class="document d-flex justify-content-between my-3">
                                ${file_marketing !== null? `<a href="{{ asset($globalVariable . 'images') }}/manager/marketing/images/${file_marketing}" download="${file_marketing}" class="btn btnGray" target="_blank"><i class="fa-solid fa-images"></i>materials</a>`: `<button class="btn btnGray" disabled><i class="fa-solid fa-images"></i>materials </button>`}
                                ${linkTaskAsString !== '' ?`<a href="${linkTaskAsString}" class="btn btnGray" target="_blank"><i class="fa-solid fa-link"></i>Link task</a>`: `<button class="btn btnGray" disabled><i class="fa-solid fa-images"></i>Link Task</button>`}
                        </label>


                        <label class="document mb-3">
                            <a href="${employee_link}" class="btn btnPrimary myTaskLink"  style="display: ${employee_link != null ?'':'none' };" target="_blank">
                    ...
                    // ---- اخفاء زرار التحكم فى حاله وصول الكارت الى 100 ----
                if (completion_percentage === '100%') {
                    const dropdownButton = card.querySelector('.dropdown-button');
                    const dropdownBox = card.querySelector('.dropdown-content');
        ...
        //  Get Json Data
        const teamMembersData = @json($teamMembersData);
        ...
        function updateCompletionPercentage(projectId, newCompletionPercentage) {
            ...
            $.ajax({
                type: 'POST',
                url: `/update-marketing/${projectId}`,
                data: {
    ...
    {{-- Upload Link Employee --}}
    <script>
        $(document).ready(function () {
            $(".createLink").on('blur', function () {
                var link = $(this).val();
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        alert(response.message + ' تم تحديث الرابط بواسطة: ' + response.updated_by);
                    },
                    error: function (xhr) {

...
class MangerController extends Controller
{
    ...
    public function storeMarketing(Request $request)
        {
            ...
            try {
                ...
                $project->linkTask = $request->input('linkTask');
        ...
        if (auth()->user()->can('isAdmin') || (auth()->user()->can('isManager') && auth()->user()->department != 'programming')) {
            ...
            foreach ($proMarket as $Market) {
                ...
                $teamMembersData[] = [
                    ...
                    "linkTask" => $Market->linkTask ,
                    "file_marketing" => $Market->file_marketing,
                    "completion_percentage" => $Market->completion_percentage,
                    "employee_link" => $Market->employee_link,
                ];
                }

            // return $teamMembersData;
            return view('manger.marketing.projectmarketing', compact('teamMembersData','proMarket'));
        }
        if (
                (auth()->user()->can('isEmployee') && auth()->user()->department != 'programming')
            ) {
                $proMarket = $projectMarketing->get();
                $userId = auth()->id();
                ...
                foreach ($proMarket as $Market) {
                    ...
                    // تحقق مما إذا كان المستخدم الحالي هو جزء من الفريق في هذا المشروع
                    $userInTeam = collect($teamMembers)->contains('id', $userId);
    ...
    public function updateMarketing(Request $request, $projectId)
    {
        ...
        if (
            ...
            (auth()->user()->can('isManager') && auth()->user()->department != 'programming') ||
            (auth()->user()->can('isEmployee') && auth()->user()->department != 'programming')
        ) {
            try {
                $project = ProjectMarketing::findOrFail($projectId);

                // التحقق إذا كان المستخدم يحاول تحديث الحالة إلى 100%
                if ($request->input('completion_percentage') == '100%') {
                    return response()->json(['error' => false, 'message' => 'غير مسموح بتحديث الحالة إلى 100%']);
                }
    ...
    // =========================== { End Update completion percentage  Project Marketing } ===========================
      ...
      // ========================={{ Start Save Link Employee }}===================================
      public function saveLink(Request $request)
        {
            $projectId = $request->input('project_id');
            $link = $request->input('employee_link');

            $project = ProjectMarketing::find($projectId);
            $project->employee_link = $link;
            $project->up_employee = auth()->user()->name;
            $project->update();

            $response = [
                'success' => true,
                'message' => 'تم حفظ الرابط بنجاح.',
                'updated_by' => $project->up_employee,
            ];

            return response()->json($response);
        }

      // ========================={{ End Save Link Employee }}===================================
